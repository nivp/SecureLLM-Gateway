...
const outputPath = process.env.ADVERSARIAL_REPORT_PATH ?? "adversarial-canary-report.html";
const jsonOutputPath = process.env.ADVERSARIAL_JSON_PATH ?? ".test-artifacts/adversarial-canary-results.json";
...
function expectedFor(testCase: AdversarialCase): { expectedStatus: number; expectedBlocked: boolean } {
...
function evaluate(testCase: AdversarialCase, actualStatus: number, actualBody: unknown, trace: CanaryTrace | undefined): boolean {
  const expected = expectedFor(testCase);
  if (actualStatus !== expected.expectedStatus) {
    return false;
  }
  if (!expected.expectedBlocked) {
    return normalizeCanaryOutput(trace?.canaryOutput) === "ok";
  }
  const body = actualBody as { threats?: unknown };
  return Array.isArray(body.threats) && body.threats.includes("llm-canary-override");
}
...
function renderHtml(results: CaseResult[]): string {
  ...
  const rows = results
    .map((result) => {
      ...
      return `<tr>
        <td><span class="pill ${result.passed ? "passed" : "failed"}">${result.passed ? "passed" : "failed"}</span></td>
        <td>${escapeHtml(result.id)}<div class="muted">${escapeHtml(result.category ?? "")}</div></td>
        ...
        <td>
          ...
          <div><strong>Threats:</strong> ${escapeHtml(result.expectedThreats.join(", ") || "n/a")}</div>
        </td>
        <td>
        ...
        <td>${result.error ? `<pre>${escapeHtml(result.error)}</pre>` : ""}</td>
      </tr>`;
    })
    .join("\n");
...
<head>
  ...
  <title>Adversarial Canary Report</title>
  <style>
...
<body>
  <main>
    <h1>Adversarial Canary Report</h1>
    ...
    <table>
      <thead><tr><th>Result</th><th>Case</th><th>User Input</th><th>Expected</th><th>Actual API</th><th>Canary LLM</th><th>Error</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </main>
</body>
...
async function main(): Promise<void> {
  ...
  const cases = JSON.parse(readFileSync(fixturePath, "utf8")) as AdversarialCase[];
  const results: CaseResult[] = [];

  for (const testCase of cases) {
    const correlationId = `adversarial-${testCase.id}-${Date.now()}`;
    const expected = expectedFor(testCase);

    try {
      const response = await postCase(testCase, correlationId);
      const trace = await readCanaryTrace(correlationId);
      const result: CaseResult = {
        ...
        canaryTrace: trace,
        passed: evaluate(testCase, response.status, response.body, trace),
        error: trace ? undefined : "No canaryTrace found in docker compose logs api. Ensure LLM_CANARY_DEBUG_LOGS=true."
      };
      results.push(result);
    } catch (error) {
      results.push({
